...
//     window (sum of all 5 row-sums in the ring), maintained across rows.
//     Per row we sub the evicted slot's row-sum and add the newly computed
//     one — two 3-bit-into-5-bit ripple chains instead of five 3-bit adds.
//
                 ...
                 size_t tile_cols)
{
    ...
    for (size_t ws = 0; ws < rw; ws += tile_words) {
        ...
        for (size_t r = row_begin; r < row_end; ++r) {
            ...
            // ---- Pass 1: read C (running 5-row sum), subtract centre, emit ----
            for (size_t vi = 0; vi < tnw; ++vi) {
                ...
                // Subtract centre ADULT bit from local copy of c.
            ...
            // ---- Compute new-row ADULT bits into adult_tmp, then update C ----
            const size_t new_row = (r + 3) % height;
            const uint64_t* sp1n = src.s1.row(new_row);
            const uint64_t* sp0n = src.s0.row(new_row);
            for (size_t vi = 0; vi < tnw; ++vi)
                vst1q_u64(adult_tmp + vi*2,
                          vandq_u64(vld1q_u64(sp1n + ws + vi*2),
                                    vld1q_u64(sp0n + ws + vi*2)));
            const size_t pw0 = (ws == 0) ? rw-2 : ws-2;
            const size_t pw1 = (ws == 0) ? rw-1 : ws-1;
            const size_t nw0 = (we == rw) ? 0 : we;
            const size_t nw1 = (we == rw) ? 1 : we+1;
            uint64_t bnd_prev_n[2] = { sp1n[pw0] & sp0n[pw0], sp1n[pw1] & sp0n[pw1] };
            uint64_t bnd_next_n[2] = { sp1n[nw0] & sp0n[nw0], sp1n[nw1] & sp0n[nw1] };

            uint64_t* const rs0t = rs0[tail];
            uint64_t* const rs1t = rs1[tail];
            uint64_t* const rs2t = rs2[tail];

            // ---- Pass 2: for each word, compute new row-sum, sub old slot, add new, store ----
            for (size_t vi = 0; vi < tnw; ++vi) {
                // Sliding-window adult bits for the new row.
                const uint64x2_t prev_v = (vi == 0)     ? vld1q_u64(bnd_prev_n)
                                                        : vld1q_u64(adult_tmp + vi*2 - 2);
                const uint64x2_t curr_v = vld1q_u64(adult_tmp + vi * 2);
                const uint64x2_t next_v = (vi == tnw-1) ? vld1q_u64(bnd_next_n)
                                                        : vld1q_u64(adult_tmp + vi*2 + 2);
                ...
                const uint64x2_t a = vorrq_u64(vshlq_n_u64(curr_v, 2), vshrq_n_u64(prev_adj, 62));
                const uint64x2_t b = vorrq_u64(vshlq_n_u64(curr_v, 1), vshrq_n_u64(prev_adj, 63));
                const uint64x2_t cc = curr_v;
                const uint64x2_t d_ = vorrq_u64(vshrq_n_u64(curr_v, 1), vshlq_n_u64(next_adj, 63));
                const uint64x2_t e_ = vorrq_u64(vshrq_n_u64(curr_v, 2), vshlq_n_u64(next_adj, 62));
                ...
                // Load old slot values (still in place before overwrite).
                const uint64x2_t r0o = vld1q_u64(rs0t + vi*2);
                const uint64x2_t r1o = vld1q_u64(rs1t + vi*2);
                const uint64x2_t r2o = vld1q_u64(rs2t + vi*2);

                // Load stored running C (untouched by Pass 1 — that pass worked on a local copy).
                uint64x2_t c0 = vld1q_u64(C0 + vi*2);
                uint64x2_t c1 = vld1q_u64(C1 + vi*2);
                uint64x2_t c2 = vld1q_u64(C2 + vi*2);
                uint64x2_t c3 = vld1q_u64(C3 + vi*2);
                ...
                // C -= (r2o, r1o, r0o)  (3-bit-into-5-bit ripple borrow)
                ...
                // C += (r2n, r1n, r0n)  (3-bit-into-5-bit ripple carry)
                uint64x2_t carry;
                ...
                // Store updated C and new row-sum into slot[tail].
                vst1q_u64(C0 + vi*2, c0);
                vst1q_u64(C1 + vi*2, c1);
                vst1q_u64(C2 + vi*2, c2);
                vst1q_u64(C3 + vi*2, c3);
